...
hardening 'sed -i   "s/^\s*account\s*required/\#account\ required/gI" /etc/pam.d/common-account;echo "account requisite   pam_tally2.so">>/etc/pam.d/common-account' 'grep "pam_tally2.so" /etc/pam.d/common-account' '"pam_tally2.so"' '"5.3.2 Ensure lockout for failed password attempts is configured (Scored)"'
